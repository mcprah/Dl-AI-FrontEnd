...
function MainPage() {
  ...
  const [isRealTime, setIsRealTime] = useState(false); // For controlling typewriter effect
  const [messages, setMessages] = useState([]);
  const chatContainerRef = useRef(null);
  const navigate = useNavigate();
  const [refreshSidebar, setRefreshSidebar] = useState(false); // For refreshing sidebar
  ...
  const handleNewChatClick = async () => {
    ...
    try {
      ...
      const response = await fetch("  https://dennislaw-backend-docker-gcr-image-615425956737.us-central1.run.app/discovery/start", {
        method: "POST",
        headers: {
  ...
  const handleSessionClick = async (sessionId) => {
    setLoading(true);
    setIsRealTime(false); // Disable typewriter effect for session history
    setMessages([]);
    ...
    try {
      ...
      const response = await fetch(
        `  https://dennislaw-backend-docker-gcr-image-615425956737.us-central1.run.app/discovery/session/?name=${parsedSessionId}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        ...
        const parsedMessages = data.turns
          .map((turn) => {
            return [
              {
                type: "user",
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await fetch("  https://dennislaw-backend-docker-gcr-image-615425956737.us-central1.run.app/discovery/chat", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      <div className="sideBar">
        <div className="upperSide">
          ...
          <Sidebar 
              ...
              refreshSidebar={refreshSidebar} // Pass the refresh trigger to Sidebar
            />
          </div>
        </div>
            ...
            <div className="chats" ref={chatContainerRef}>
              {messages.length === 0 ? (
                ...
                messages.map((message, index) => (
                  <div
                    className={message.type === "user" ? "chat" : "chat bot"}
                    key={index}
                  >
                    <img
                      className="chatImg"
                      src={message.type === "user" ? userIcon : AILogo}
                      alt={message.type === "user" ? "User" : "Bot"}
                    />
                    <p className="txt">
                      {message.type === "bot" && message.isLoading ? (
                        <div className="ai-loader"></div>
                      ) : message.type === "bot" && isRealTime ? (
